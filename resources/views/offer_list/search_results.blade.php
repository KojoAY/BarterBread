    ...
    @section('content')
            <section class="main-content-block">
                ...
                <section class="list-block">
                    ...
                    <h1 id="title">Results for {{ $_GET['of'] }}</h1>

                    @if($getSearchResults->count() == 0)
                        <article style="text-align: center; padding: 20px 0;">
                            Sorry, no results for <strong>{{ $_GET["of"] }}</strong>. <a href="{{ url('/listoffer') }}">List an Offer</a>
                        </article>
                    @else

                    <ul class="list-ul">
                        @foreach($getSearchResults as $offerList)
                        <li>
                            <section class="photo">
                                <?php
