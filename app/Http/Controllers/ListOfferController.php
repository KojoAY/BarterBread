...
class ListOfferController extends Controller
{
    public function showListOfferForm()
	{
		...
		if (Session::has("traderToken")) {
	        ...
	        $getTraderContacts = DB::table('bb_account')
	        	->select('*')
	        ...
	        $data = array(
            	"pageTitle" => "List Offer - " . config('app.name', 'BarterBread'),
	        	"getMainCategories" => $getMainCategories,
	        	"getTraderContacts" => $getTraderContacts
	        );
	...
	public function listOffer(Request $request)
	{
		...
		if (Session::has("traderToken")) {
			...
			$oCountry = "GH"; //$request->get("o_country");
			$oZipCode = "00233"; //$request->get("o_zipcode");
